...
def finish():#When the user is finished
    ...
    if widgets['plays fortnite'].get().lower() == 'yes':
        ...
        allowed = False
            ...
            if widgets['favourite anime'].get().lower() == 'jojo':
                allowed = False
    elif widgets['favourite game'].get().lower() == 'genshin impact':
        allowed = False
    ...
    code = ''.join([str(randint(0, 9)) if randint(0, 1) == 0 else choice(ascii_lowercase) if randint(0, 1) == 0 else choice(ascii_uppercase) for i in range(32)])
